...
async function uploadImage(imageUrl, tokenManager) {
  ...
  if (imageUrl.startsWith('http://') || imageUrl.startsWith('https://')) {
    ...
    throw new Error(`只支持网络图片 URL：${imageUrl}`);
  }

  const formData = new FormData();
  formData.append('media', imageData, { filename: 'image.jpg', contentType });
...
// 上传封面图
async function uploadCoverImage(tokenManager) {
  const accessToken = await tokenManager.getAccessToken();
  ...
  throw new Error(`上传封面图失败：${response.data.errmsg}`);
}
...
async function processContent(content, isMarkdown, tokenManager) {
  ...
  if (isMarkdown) {
    ...
    for (const match of matches) {
      ...
      if (imageUrl.startsWith('http://') || imageUrl.startsWith('https://')) {
        try {
          const result = await uploadImage(imageUrl, tokenManager);
          imageCount++;
  ...
  // 纯文本处理
  const html = `<p style="${buildStyleString(DEFAULT_STYLES.p)}">${content.replace(/\n/g, '<br>')}</p>`;
  return { html, imageCount };
}
...
async function pushDraftText({ content, title, digest, sourceUrl, isMarkdown = true }) {
  if (!content || !title) {
    throw new Error('content 和 title 是必填参数');
  }
  ...
  // 处理内容
  const { html, imageCount } = await processContent(content, isMarkdown, tokenManager);

  // 上传封面图
  console.log('正在上传封面图...');
  const thumbMediaId = await uploadCoverImage(tokenManager);
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    console.log('Usage: node push-draft-text.js <content> <title> [digest] [sourceUrl] [isMarkdown]');
    console.log('');
    console.log('Examples:');
    console.log('  node push-draft-text.js "文章内容" "文章标题"');
    console.log('  node push-draft-text.js "# Markdown内容" "标题" "" "" true');
    process.exit(1);
  }

  const [content, title, digest, sourceUrl, isMarkdownStr] = args;

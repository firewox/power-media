...
Power Media 是一套社交媒体自动化工具集，通过 **Claude Code / OpenCode Skills 系统**，结合**截图识别 + 多模态 AI + Computer Use** 与**微信公众号官方 API 增强封装**，实现多平台社交媒体的内容发布与管理。
...
用户指令 → Claude Code / OpenCode
              ...
              └── Skills 系统（场景化能力模块）
                    ...
                    ├── 浏览器自动化组
                    │   ├── 微博 (Weibo)
                    │   ├── 小红书 (RedNote)    ← 截图识别 + 多模态 AI + Computer Use
                    │   └── 知乎 (Zhihu)
                    │
                    ├── 微信公众号 (WeChat)      ← 官方 API（增强封装）
                    │
                    └── check-ip                ← 本机出网 IP 检测

计算机控制基础设施（为浏览器自动化组提供计算机操作能力）：
...
| 模式 | 适用平台 | 技术方案 |
|------|----------|----------|
| **截图 + 多模态 + Computer Use** | 微博、小红书、知乎 | Agent 截图 → 多模态 AI 识别 UI → computer-mcp 模拟操作 |
| **API 直连（增强封装）** | 微信公众号 | 微信官方 HTTP API + 主题定制 + 功能增强 |
| **简易工具** | check-ip | HTTP 请求查询本机出网 IP |

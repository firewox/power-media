...
1. 检查 `wechat/wechat-config.json` 文件是否存在
2. 检查环境变量 `WECHAT_APP_ID` 和 `WECHAT_APP_SECRET` 是否已设置
...
2. **提供配置文件** - 在 `wechat/` 目录下创建 `wechat-config.json` 文件
...
1. `wechat/wechat-config.json`（推荐）
2. `.env` 文件
